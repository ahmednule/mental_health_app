...
, [
  ...
  const getMeetingLink = () => {
    ...
    return `https://elevateminds-ns.netlify.app/smeeting/${randomString}`;
  }
  useEffect(() => {
    if(selectedHour !== null){
     ...
     // api call to check if the hour is available -api/auth/counselor/bookings/precheck
      const url = `/api/auth/peerCounselor/PeerBooking/precheck`;
      const options = {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
  ...
  const handleBooking = async (e: any) => {
    ...
    setIsBooking(true);
    const res = await fetch(`/api/auth/peerCounselor/PeerBooking`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
